...
class DatabaseHelper {
  ...
  static Future<Database> _getDB(String dbName) async {
    return openDatabase(
      ...
      onCreate: (db, version) async => await db.execute(
        "CREATE TABLE $dbName(id INTEGER PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL, antonyms TEXT NOT NULL, synonyms TEXT NOT NULL);",
      ),
      version: _version,
    );
  }

  static Future<int> addNote(String dbName, LikedFlashCardsModel note) async {
  ...
  static Future<int> updateNote(String dbName, LikedFlashCardsModel note) async {
    final db = await _getDB(dbName);
    return await db.update(
      dbName,
      note.toJson(),
  ...
  static Future<int> deleteNote(String dbName, LikedFlashCardsModel note) async {
    final db = await _getDB(dbName);
    return await db.delete(
      dbName,
      where: 'id = ?',
  ...
  static Future<void> deleteAllData(String dbName) async {
    final db = await _getDB(dbName);
    await db.delete(dbName);
  }

  static Future<bool> isItemExists(String dbName, LikedFlashCardsModel item) async {
    final db = await _getDB(dbName);
